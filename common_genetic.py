...
from common_schedule import schedule_cost, CommonSchedule, conflict


class CommonGeneticOptimize:
    """Genetic Algorithm.
        popsize: 种群数量
        mutprob: 变异概率
        crossProb: 交叉概率
    ...
    def checkIsBetter(self, p):
        ...
        count = 0
        if len(roomSet) / CommonSchedule.roomRange > 0.80:
            count += 1
        if len(slotSet) / CommonSchedule.slotInDay > 0.80:
            count += 1
        if len(weekSet) / CommonSchedule.dayInWeek > 0.80:
            count += 1
        if count == 3:
            return 1
        else:
    ...
    def init_population(self, schedules, roomRange):
        ...
        while len(self.population) < self.popsize:
            ...
            if conflictCount == 0:
                print('find item,fitness score=' + str(common_schedule.fitness(entity)))
    ...
    def dealConflict(self, entity, conflict1, conflict2, conflict3):
        ...
        # 记录所有教室
        for i in range(CommonSchedule.roomRange):
            s = [[0 for i in range(CommonSchedule.slotInDay)] for i in range(CommonSchedule.dayInWeek)]
        ...
        for roomId in roomId2UsedSpace.keys():
            ...
            freeSpace = []
            for i in range(CommonSchedule.dayInWeek):
                for j in range(CommonSchedule.slotInDay):
                    if usedMatrix[i][j] == 0:
    ...
    def fixConflict(self, conflictList, entity, roomId2FreeSpace):
        for item in conflictList:
            ...
            randomRoomId = np.random.randint(1, CommonSchedule.roomRange + 1, 1)[0]
    ...
    def getAvailableRoom(self, roomId2FreeSpace, nowroomId, deep):
        if deep == CommonSchedule.roomRange:
            # 没有任何一个教室有空间
            return -1
        ...
        if len(array) > 0:
            ...
            if nowroomId + 1 > CommonSchedule.roomRange:
                nowroomId = 1
            else:
                nowroomId += 1
    ...
    def mutateOp(self, newPopulation, index):
        ...
        while True:
            ...
            for p in item:
                ...
                if pos == 0:
                    # p.roomId = self.addSub(p.roomId, operation, roomRange)
                    p.roomId = self.randomWithOut(CommonSchedule.roomRange, p.roomId)
                if pos == 1:
                    # p.weekDay = self.addSub(p.weekDay, operation, Schedule.dayInWeek)
                    p.weekDay = self.randomWithOut(CommonSchedule.dayInWeek, p.weekDay)
                if pos == 2:
                    # p.slot = self.addSub(p.slot, operation, Schedule.slotInDay)
                    p.slot = self.randomWithOut(CommonSchedule.slotInDay, p.slot)
            ...
            if conflictCount1 > 0 and mutateCount<CommonSchedule.mutateRetryMaxCount:
    ...
    def crossoverOp(self, eiltePopulation, e1, e2):
        ...
        while True:
            ...
            if (conflictCount1 > 0 or conflictCount2 > 0) and crossCount<CommonSchedule.crossRetryMaxCount :
                # 交叉后出现冲突，则重新选取交叉位点
                continue
            else:
                break
    ...
    def evolution(self):
        ...
        for i in range(self.maxiter):
            ...
            if (bestConflict == 0 and bestFitness == maxFitness and appearCount >= CommonGeneticOptimize.maxAppearCount) or (
                    i + 1 == self.maxiter):
                # 结束，输出最优解
