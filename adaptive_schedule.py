...
def random_init(self, roomRange):
    ...
    """
    self.roomId = np.random.randint(1, 1 + AdaptiveSchedule.roomRange, 1)[0]
    self.weekDay = np.random.randint(1, 1 + AdaptiveSchedule.dayInWeek, 1)[0]
    self.slot = np.random.randint(1, 1 + AdaptiveSchedule.slotInDay, 1)[0]
    ...
    while len(classCourse2TimeTable) > 0:
        ...
        for i in soredArrayIndex:
            ...
            if pre is None:
                ...
                gap = (now.weekDay - pre.weekDay) * AdaptiveSchedule.slotInDay + now.slot - pre.slot
                sum += gapScore(gap)
            count += 1
            pre = now
    return sum / count
...
def fitness(p):
    ...
    s1 = f1(p)
    # 计算教室利用率
    s2 = f2(p) / (AdaptiveSchedule.dayInWeek * AdaptiveSchedule.slotInDay)
    # 计算自习模型
    s3 = f3(p)
    # 节次最优模型
    s4 = f4(p)
...
def fitness2(p):
    ...
    # 节次最优模型
    s4 = f4(p)
    score = (k1 * s1 + k2 * s2 + k3 * s3 + k4 * s4) * AdaptiveSchedule.k
    # print('s:{} | s1:{} | s2:{} | s3:{} | s4:{}'.format(score,s1, s2, s3,s4))
    return score
